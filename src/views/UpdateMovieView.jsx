...
const UpdateMovieView = () => {
  ...
  const onSubmit = async (data) => {
    const id = location.state.movieProp.movieId;
    try {
      const saveOk = await movieService.update(id, data);
      if (saveOk) {
        Swal.fire({
          icon: "success",
          title: "Película Actualizada",
          text: `"${data.name}" se actualizó exitosamente`,
    ...
    <div className="min-h-screen bg-[#1a1d29] py-8 px-4">
      <div className="max-w-3xl mx-auto">
        <div className="bg-[#252836] rounded-lg shadow-xl p-8">
          <h2 className="text-3xl font-bold text-white mb-6">Actualizar: "{location.state.movieProp.name}"</h2>
